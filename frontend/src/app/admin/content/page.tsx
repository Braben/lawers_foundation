...
export default function ContentEngine() {
  ...
  const publish = async () => {
    ...
    try{
      ...
      setMsg(`Published as ${type.toUpperCase()} ✓`);
      setTitle(''); setExcerpt(''); setContent(''); setImage('');
    }catch(e: unknown){ setMsg(errorMessage(e)); } finally{ setBusy(false); }
  };
    ...
    <div className="space-y-6 max-w-[1100px]">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500">Publish blog posts and impact stories with images from the media library.</p>
        </div>
      ...
      <div className="grid lg:grid-cols-[1.6fr_0.9fr] gap-6">
        <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm space-y-4">
          ...
          <RichTextEditor disabled={!canManage} value={content} onChange={setContent} placeholder="Write your story… Use toolbar for bold, lists, quotes, links"/>
          ...
          <button onClick={publish} disabled={busy || !canManage} className="w-full py-3 rounded-xl bg-[#2C5F2D] text-white font-semibold disabled:opacity-50">{busy?'Publishing…':`Publish as ${type}`}</button>
          {msg && <div className="text-sm p-3 rounded-xl bg-[#EDF4F2] border border-[#97BC62]">{msg}</div>}
          <div className="text-xs text-gray-400">Available to publishers and administrators.</div>
        </div>
