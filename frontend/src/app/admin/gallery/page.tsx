...
import type { PendingImage } from '@/lib/direct-upload';
...
export default function GalleryManager(){
  ...
  const reset=()=>{setForm(empty);setFiles([]);setPending([]);setProgress('');setEditing(null);if(input.current)input.current.value='';};
  const save=async(event:FormEvent)=>{
    ...
    try{
      if(editing) await api.update('gallery',editing.id,{title:form.title,description:form.description,category:form.category,...(editing.type==='video'?{url:form.url,thumbnail:form.thumbnail||'/images/lawersfoundation.png'}:{})});
        ...
        const result=await api.uploadImages(files,{title:form.title,description:form.description,category:form.category},(done,total)=>setProgress(`${done} of ${total} images processed`),pending.length?pending:undefined);
        if(result.failed.length){
          setPending(result.failed);setFiles(result.failed.map(job=>job.file));
          setItems(await api.getGallery());
          setMessage(`${result.items.length} saved; ${result.failed.length} need retry. ${result.failed.map(job=>`${job.file.name}: ${job.message}`).join(' ')}`);
          return;
        }
      }
      reset();setMessage('Gallery saved. New media is available on the public gallery.');
      try {setItems(await api.getGallery());} catch {setMessage('Gallery saved, but the list could not refresh. Reload this page to see the saved media.');}
  ...
  return <div className="space-y-6"><div><h1 className="text-2xl font-bold">Gallery</h1><p className="text-sm text-gray-500">Upload multiple images or add videos visitors can play on the website.</p></div>
    {canManage&&<form onSubmit={save} className="bg-white rounded-2xl p-5 border shadow-sm space-y-4 max-w-2xl"><fieldset disabled={busy} className="space-y-4">
      ...
      {!editing&&mode==='image'&&<label className="block text-sm">Images (up to 20; JPEG, PNG or WebP; 5 MB each)<input ref={input} multiple type="file" accept="image/jpeg,image/png,image/webp" required onChange={e=>{setFiles(Array.from(e.target.files||[]));setPending([]);}} className="block mt-2 w-full border rounded-xl p-3"/>{files.length>0&&<span className="block mt-2">{files.length} selected: {files.map(f=>f.name).join(', ')}</span>}</label>}
      ...
      <label className="block text-sm">{!editing&&mode==='image'?'Title prefix (optional; otherwise use filenames)':'Title'}<input required={!!editing||mode==='video'} disabled={pending.length>0} minLength={2} maxLength={140} value={form.title} onChange={e=>setForm({...form,title:e.target.value})} className="block mt-1 w-full border rounded-xl px-3 py-2"/></label>
      <label className="block text-sm">Description<textarea disabled={pending.length>0} maxLength={2000} value={form.description} onChange={e=>setForm({...form,description:e.target.value})} className="block mt-1 w-full border rounded-xl px-3 py-2" rows={3}/></label>
      <label className="block text-sm">Category<select disabled={pending.length>0} value={form.category} onChange={e=>setForm({...form,category:e.target.value})} className="block mt-1 w-full border rounded-xl px-3 py-2">{['community','education','empowerment','caregivers','videos'].map(c=><option key={c} value={c}>{c}</option>)}</select></label>
      <div className="flex gap-3"><button className="px-4 py-2 rounded-xl bg-[#2C5F2D] text-white disabled:opacity-50">{busy?(progress||'Preparing upload...'):pending.length?'Retry remaining images':editing?'Save changes':mode==='image'?'Upload images':'Add video'}</button>{(editing||pending.length>0)&&<button type="button" onClick={reset} className="px-4 py-2 border rounded-xl">Cancel</button>}</div>
    </fieldset></form>}
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">{items.map(item=><article key={item.id} className="bg-white rounded-xl border overflow-hidden"><LazyImage src={item.thumbnail||item.url} alt={item.title} aspectRatio="video"/><div className="p-4 space-y-2"><h2 className="font-semibold">{item.title}</h2><p className="text-sm text-gray-500">{item.description}</p><p className="text-xs text-gray-500">{item.category} · {item.type}</p><div className="flex gap-3">{item.type==='video'&&<button onClick={()=>setPreview(item)} className="underline text-sm">Play</button>}{canManage&&<><button disabled={busy} onClick={()=>{setPending([]);setEditing(item);setForm({title:item.title,description:item.description,category:item.category,url:item.url,thumbnail:item.thumbnail});}} className="text-sm underline">Edit</button><button disabled={busy} onClick={()=>remove(item)} className="text-sm text-red-700 underline">Remove</button></>}</div></div></article>)}</div>
