...
export default function EventManager() {
  ...
  const load = ()=>{
    fetch(`${process.env.NEXT_PUBLIC_API_URL||'http://localhost:4000'}/api/events`).then(r=>r.json()).then(j=> setEvents(j.data||[])).catch(()=>{});
  };
  useEffect(()=>{ load(); },[]);
  ...
  const create = async (e: React.FormEvent)=>{
    ...
    try{
      const payload:any = {
        ...
        capacity: Number(form.capacity), registeredCount:0, category: form.category
      };
      await api.create('events', payload);
      setMsg('Event created — wired to POST /api/events');
  ...
  const openRsvps = async (ev:any)=>{
    setSelected(ev); setRsvps([]);
    const token = localStorage.getItem('lf_token');
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL||'http://localhost:4000'}/api/events/${ev.id}/rsvps`, { headers: token?{Authorization:`Bearer ${token}`}:{} as any });
    const j = await res.json(); setRsvps(j.data||[]);
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500">Create events with capacity → track RSVPs → cap sign-ups. Wired to <code>POST /api/events</code> & <code>POST /api/events/:id/rsvp</code></p>
      </div>
      <div className="grid lg:grid-cols-[0.95fr_1.2fr] gap-6">
        <form onSubmit={create} className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm space-y-3">
        ...
        <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
            ...
            {events.map(ev=>(
              <div key={ev.id} className="p-3 rounded-xl border flex items-center justify-between gap-3 hover:bg-gray-50">
                <div className="min-w-0">
                  ...
                  <div className="text-xs text-gray-500">{ev.date} • {ev.location?.name||ev.location} • {ev.registeredCount||0}/{ev.capacity||100} registered</div>
          ...
          {selected && (
            <div className="mt-4 p-3 rounded-xl bg-[#F8FAF8] border">
              ...
              <div className="text-xs text-gray-500 mb-2">Capacity capped: {selected.capacity} • {rsvps.length} registered • {Math.max(0,(selected.capacity||100)-rsvps.reduce((s:number,r:any)=>s+Number(r.guests||1),0))} spots left</div>
              <div className="overflow-x-auto">
                ...
                {!rsvps.length && <div className="text-xs text-gray-400 py-4 text-center">No RSVPs yet — public registers via POST /api/events/:id/rsvp (capacity-checked)</div>}
              </div>
            </div>
          )}
        </div>
