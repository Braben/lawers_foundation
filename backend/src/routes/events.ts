...
const rsvpSchema = z.object({ name: z.string().trim().min(2).max(150), email: z.string().trim().email().max(254), phone: z.string().optional(), guests: z.number().int().min(1).max(100).default(1) });
