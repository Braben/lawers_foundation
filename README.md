...
Main administrators configured by `ADMIN_EMAILS` or `SUPER_ADMINS` retain protected full access. CEO has every permission by default. Managers initially manage content, gallery and events, and view analytics. Publishers initially manage content and gallery; event managers manage events. Unassigned accounts have no staff permissions.
